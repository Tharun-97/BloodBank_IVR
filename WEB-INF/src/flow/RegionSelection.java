...
public class RegionSelection extends com.avaya.sce.runtime.Menu {
	...
	public void requestBegin(SCESession mySession) {
		...
		try {
			...
			regionlist=(ArrayList<String>) mySession.getVariableField(IProjectVariables.API_RESULTS, IProjectVariables.API_RESULTS_FIELD_REGION_LIST).getObjectValue();
		
			TraceInfo.trace(ITraceInfo.TRACE_LEVEL_INFO,"Regions List added:"+regionlist, mySession);
		
		} catch (Exception e) {
			TraceInfo.trace(ITraceInfo.TRACE_LEVEL_ERROR,"Error occoured in Region selection node: "+e.getLocalizedMessage(), mySession);
		}
	...
	public void updatePrompts(Collection prompts, SCESession mySession) {
		...
		for (int i = 0; i < regionlist.size(); i++) {
			...
			c.Choice(region, dtmf, mySession, p);
	...
	public void updateChoices(Collection choices, SCESession mySession) {
		...
		try {
			...
			for (int i = 0; i < regionlist.size(); i++) {
				...
				choice = new com.avaya.sce.runtime.Choice(region, dtmf, true, "exact","BranchAPI", grammarInfo, true);
				choices.add(choice);
